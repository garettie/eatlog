...
- **Two selection controls, two looks.** A *segmented control* switches between a few adjacent modes or values in place (meal period, Consumed/Remaining, 1M–1Y, the kg/lb toggle beside a typed weight, Servings/Grams, Calculated/Custom) and shows a sliding white thumb on a tonal track (`SegmentedControl`, `density="compact"` for the meal bar). A *choice list* asks the user to pick one item as a setting (goal, sex, units, activity level, portion shortcuts, serving options): the selected card or chip steps up one surface tone with a 2dp White Action outline and ink text. Goal, sex, and units share `ChoiceCards` in onboarding and Profile. Never give a choice list the sliding thumb or a segmented control outlined chips.
