...
import type {
	...
	OwnershipResult,
} from "./src/services/dataOwnership.types";
...
export default function App() {
	...
	const runDataMaintenance = useCallback(
		(label: string, task: MaintenanceTask) =>
				...
				<ResponsiveContent maxWidth={520} className="items-center">
					...
					{dbError && !fontError && (
						<Pressable
							...
							className="mt-5 min-h-[48px] justify-center rounded-full bg-m3-surface-container-high px-6"
						>
							<Text className="font-semibold text-sm text-m3-on-surface">
								Retry database
							</Text>
						</Pressable>
					)}
				...
				{maintenance ? (
					<View className="flex-1 items-center justify-center bg-m3-surface px-8">
						<ResponsiveContent maxWidth={520} className="items-center">
							{maintenanceError ? (
								<>
									<Text className="text-center font-semibold text-lg text-m3-on-surface">
										{maintenance.label} failed
									</Text>
									<Text
										accessibilityLiveRegion="assertive"
										className="mt-3 text-center text-sm text-m3-error"
									>
										{maintenanceError}
									</Text>
								...
								<>
									...
									<Text className="mt-5 text-center font-semibold text-lg text-m3-on-surface">
										{maintenance.label}
									</Text>
									<Text
										accessibilityLiveRegion="polite"
										className="mt-2 text-center text-sm text-m3-on-surface-variant"
									>
					...
					<View className="flex-1 items-center justify-center bg-m3-surface px-8">
						<ResponsiveContent maxWidth={520} className="items-center">
							<View className="h-14 w-14 items-center justify-center rounded-full bg-m3-primary-container">
								<Text className="font-bold text-xl text-m3-on-primary-container">
									✓
								</Text>
							</View>
							<Text className="mt-5 text-center font-semibold text-lg text-m3-on-surface">
