...
| Camera | Permission is requested only after Camera is selected. Denial, cancellation, launch failure, unreadable image, and retry states are explicit. A declined first-use transmission gate uploads nothing. | Grant/deny/retry and resume testing. **PHYSICAL DEVICE** |
...
| Image preparation and request | Image preparation stays local until the one-time transmission gate passes. Worker estimation has a 22-second abort timeout and sanitized failures. | Memory-pressure and slow-network behavior. **PHYSICAL DEVICE** |
...
- `env TMPDIR=/tmp npm test`: passed 203 tests, including installation creation/concurrency/persistence/corruption/failure, backup exclusion, Android-only feature selection, and restored Health Connect device-state cleanup.
