...
8. Open Profile → Plan to inspect status, localized products, fair-use counters, Support ID, restore, and management actions. Then open Backup and restore, create an `.eatlog-backup`, inspect a supported archive, and confirm twice before replacement. Open Export data to create readable, non-restorable CSV files without entitlement state.
