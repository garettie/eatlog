...
The September 24 owner screenshots now fill the homepage's Today, Add, review, Diary, weight, and calorie positions; story step 03 uses the weight chart on desktop and phones.

`assets/punk-fire.png` is the fire behind the hero's FOSS. BYOK. line: "Fire" by i_rk_jain on OpenGameArt (CC0), restacked into a transparent 48 by 384 vertical strip of 12 frames. Each source was 810 by 1800 pixels; the website crop removes the top 80 pixels, leaving 810 by 1720 without the status bar. Responsive aliases are 405 by 860. The final image in story step 04 remains `planscreen3.jpg`/`planscreen3-405.jpg` at the owner's request.
