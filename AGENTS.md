...
- Date-selector wheels use non-virtualized `ScrollView` rows with explicit `snapToOffsets`; track the latest offset, debounce a post-drag fallback, then normalize and commit the centered row because Android native snapping and momentum-end alone are not reliable.
