...
- Bottom-sheet state changes use a 200ms shell, 90ms exit, and 150ms entrance; keep wrapper identity and dynamic-sizing mode stable through an open flow so in-flight refs survive, and prevent Gorhom resize gaps by extending the background beneath its clipped container.
...
- Expanded review section titles share the same title role; the shared white-thumb Servings/Grams toggle sits left of its amount editor in one row, grams entry is one outlined tonal field whose value stays centered independently of its integrated `g` suffix, amount shortcuts use canonical no-space units and a clear selected state, and multi-serving summaries show total grams rather than the per-unit weight.
