...
- Owner and friend testing uses the standalone EAS `preview` APK named `Eatlog Preview` with package `com.sgaret.eatlog.preview` and channel `subscription-preview`; do not prescribe the development-client APK because it requires Metro. RevenueCat Test Store must use the standalone debug APK (`:app:assembleDebug` plus `EX_UPDATES_NATIVE_DEBUG=1`, no development client); a release APK with a `test_` key crashes by design. Isolate unfinished subscription work behind RevenueCat Test Store and a staging Worker, and never publish a native-dependent billing change to an older preview runtime through EAS Update.
