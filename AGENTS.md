...
- Android date selection uses imperative `DateTimePickerAndroid.open` with `display: 'spinner'` and explicit `Set date`/`Cancel` labels; never reintroduce JavaScript `ScrollView` wheels, and let the native spinner resolve readable button colors from its dialog theme.
