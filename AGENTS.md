...
- The owner's API credentials are developer-provisioned at build time; never ask users to enter, view, or manage them. The only user-managed credential is a Manok user's own Google AI Studio key: SecureStore only, shown only as its first and last four characters and only on the Profile → AI estimates screen (the Profile overview row says `My key`), never in SQLite, backups, exports, logs, or Worker requests.
