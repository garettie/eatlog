    ...
    <View className="flex-row bg-m3-surface-container-high rounded-full p-1.5">
      {MEALS.map((m) => (
        <Pressable
          ...
          className={`flex-1 py-3.5 rounded-full items-center ${value === m.value ? 'bg-m3-surface-container' : ''}`}
        >
          <Text
            className={`text-xs font-semibold ${value === m.value ? 'text-m3-on-surface' : 'text-m3-on-surface-variant'}`}
          >
            {m.label}
          </Text>
        </Pressable>
