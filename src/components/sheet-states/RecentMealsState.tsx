...
export default function RecentMealsState({
	onSelectMeal,
	onBack,
}: RecentMealsStateProps) {
	...
	useEffect(() => {
		...
		setLoading(true);
		setError(false);
		void getLoggedMeals(deferredQuery)
			.then((items) => {
	...
	const handleTogglePin = useCallback(async (meal: LoggedMeal) => {
		...
		const updateMeal = (item: LoggedMeal, pinned: number) =>
			item.food_key === meal.food_key ? { ...item, is_pinned: pinned } : item;
		const sortMeals = (items: LoggedMeal[]) =>
			items.sort(
				(first, second) =>
					second.is_pinned - first.is_pinned ||
					second.last_logged_at.localeCompare(first.last_logged_at),
			);

		setMeals((current) =>
			sortMeals(current.map((item) => updateMeal(item, nextPinned ? 1 : 0))),
		);
		try {
			...
			setMeals((current) =>
				sortMeals(current.map((item) => updateMeal(item, meal.is_pinned))),
			);
		}
	}, []);

	const renderMeal = useCallback(
		({ item: meal }: { item: LoggedMeal }) => (
			<NutritionCard
				name={meal.meal_name}
				...
				action={
					<Pressable
						...
						accessibilityLabel={
							meal.is_pinned
								? `Unpin ${meal.meal_name}`
								: `Pin ${meal.meal_name}`
						}
						className="absolute right-2 top-2 w-12 h-12 items-center justify-center active:opacity-60"
					>
						<MaterialIcons
							name={meal.is_pinned ? "favorite" : "favorite-border"}
							size={20}
							color={meal.is_pinned ? M3.error : M3.onSurfaceVariant}
						/>
					</Pressable>
				}
			/>
	...
	const emptyContent = loading ? (
		...
		<View className="py-12 items-center" accessibilityLiveRegion="assertive">
			...
			<Pressable
				...
				accessibilityRole="button"
				accessibilityLabel="Retry loading recent meals"
				className="min-h-[48px] mt-3 justify-center rounded-full bg-m3-surface-container-high px-5 active:opacity-60"
			>
		...
		<View className="py-12 items-center gap-2">
			...
			<Text className="text-m3-on-surface-variant text-sm font-medium">
				{query ? "No meals found" : "No meals logged yet"}
			</Text>
		</View>
	);
