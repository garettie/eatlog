...
export default function SearchInputState({
	...
	const handleToggleFoodPin = useCallback(
		async (food: FoodResult) => {
			...
			await Promise.all(
				[...new Set(keys)].map((key) => setFoodPinned(key, nextPinned)),
			).catch((error) => {
	...
	const foodRow = (food: FoodResult) => (
		<FoodSearchResultRow
			...
			onTogglePin={
				food.history ? () => void handleToggleFoodPin(food) : undefined
			}
	...
	return (
		<View className="flex-1">
			...
			>
					...
					<View className="pt-3 gap-2">
						{search.remoteState !== "unavailable" ? (
							<Pressable
								...
								disabled={estimating}
								accessibilityRole="button"
								className="min-h-[48px] flex-row items-center justify-center gap-2 rounded-full bg-m3-surface-container-high active:opacity-60 disabled:opacity-50"
							>
								{estimating ? (
									<ActivityIndicator size="small" color={M3.onSurfaceVariant} />
								) : (
				...
				{quickLogError ? (
					<Text
						className="text-m3-error text-sm px-1"
						accessibilityLiveRegion="assertive"
					>
						{quickLogError}
					</Text>
