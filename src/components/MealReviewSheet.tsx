...
export default function MealReviewSheet({ result, visible, onClose, onLogComplete }: MealReviewSheetProps) {
  ...
  const canClose = useCallback(() => {
    if (loggedRef.current) { loggedRef.current = false; dirtyRef.current = false; return true; }
    ...
    try {
      ...
      for (const comp of components) {
        ...
        if (comp.food.source === 'describe' || comp.food.source === 'scan') {
          await cacheFoodItem({
            name: comp.food.name, normalizedName: comp.food.normalizedName, brand: comp.food.brand,
            preparation: comp.food.preparation, calories_per_100g: comp.per100g.calories,
            protein_g_per_100g: comp.per100g.protein, carbs_g_per_100g: comp.per100g.carbs,
            fat_g_per_100g: comp.per100g.fat, serving_size_g: comp.servingSizeGrams ?? comp.grams,
            serving_label: comp.servingLabel, source: comp.food.source as 'scan' | 'describe',
          });
        }
      }
  ...
  return (
    <SlideModal visible={visible} onClose={onClose} height="90%" canClose={canClose}>
      <View className="flex-1">
        <ScrollView className="flex-1 px-5" contentContainerClassName="pb-8 gap-4" keyboardShouldPersistTaps="handled">
                    ...
                    {(['calories', 'protein', 'carbs', 'fat'] as const).map((field) => (
                      <View key={field} className="flex-1 gap-1">
                        ...
                        <TextInput
                          ...
                          onChangeText={(t) => updatePer100g(idx, field, parseFloat(t) || 0)}
        ...
        {removed && (
          <View className="absolute bottom-4 left-5 right-5 bg-m3-surface-container-highest rounded-2xl px-4 py-3 flex-row items-center border border-m3-outline-variant/30">
