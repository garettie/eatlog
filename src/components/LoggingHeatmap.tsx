...
function LoggingHeatmap({
    ...
    const accessibilityLabel = `${copy.title}, 30-day history. ${model.windowCount} days logged. ${model.currentWeekCount} of 7 days logged this week.`;
                ...
                {model.rows.map((row) => (
                    <View key={row[0].date} className="flex-row gap-1">
                        {row.map((cell) => (
                            <View
                                key={cell.date}
                                className={`flex-1 aspect-square rounded-sm ${cell.logged
