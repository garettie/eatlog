...
export default function FoodSearchResultRow({
  ...
  const calories = food.history?.calories
    ?? (food.caloriesPer100g != null && portion ? food.caloriesPer100g * portion.grams / 100 : null);
  ...
  return (
    <View className="min-h-[72px] flex-row items-center rounded-2xl bg-m3-surface-container border border-m3-outline-variant/30 overflow-hidden">
      ...
      >
        <View className="flex-row items-start justify-between gap-3">
          <View className="flex-1">
            <Text className="text-m3-on-surface text-sm font-semibold" numberOfLines={2}>{food.name}</Text>
            <Text className="text-m3-on-surface-variant text-xs mt-0.5" numberOfLines={1}>{context}</Text>
            {portionText ? <Text className="text-m3-on-surface-variant text-compact mt-0.5" numberOfLines={1}>{portionText}</Text> : null}
          </View>
      ...
      {food.history && onTogglePin ? (
        ...
        >
          <MaterialIcons name={food.isPinned ? 'favorite' : 'favorite-border'} size={20} color={food.isPinned ? M3.error : M3.onSurfaceVariant} />
        </Pressable>
      ) : null}
      {food.history && onQuickLog ? (
        ...
        >
          {quickLogging
            ...
            : <MaterialIcons name="add-circle-outline" size={21} color={M3.onSurface} />}
        </Pressable>
      ) : null}
