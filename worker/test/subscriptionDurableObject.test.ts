...
for (const [label, make] of stores()) {
  test(`${label}: concurrent requests sharing one ID reserve one charge and report the rest as duplicates`, async () => {
    ...
    const decisions = await Promise.all(Array.from({ length: 10 }, () => (
      store.reserve(who, 'one-logical-action', NOW)
    )));
    ...
    const usage = await store.usage(who, NOW);
  ...
  test(`${label}: a refund arriving after a finalize cannot undo the completed charge`, async () => {
    ...
    await store.reserve(finished, 'completed', NOW);
    ...
    const afterFinalize = await store.usage(refunded, NOW);
  ...
  test(`${label}: five distinct failed requests reach the refund ceiling`, async () => {
    ...
    for (let index = 0; index < 5; index += 1) {
      const decision = await store.reserve(who, `failed-${index}`, NOW + index);
      assert.equal(decision.allowed, true);
      await store.refund(who, `failed-${index}`, 'unrecognized');
    }

    const blocked = await store.reserve(who, 'failed-5', NOW + 5);
    assert.equal(blocked.allowed, false);
    assert.equal(blocked.code, 'REFUND_DAILY_LIMIT');
    // None of it touched the visible allowance.
    const usage = await store.usage(who, NOW + 5);
    assert.equal(usage.kind === 'paid' && usage.remaining24Hours, 30);
  });
  ...
  test(`${label}: retrying one failed request ID is one submission, not five`, async () => {
    ...
    for (let index = 0; index < 6; index += 1) {
      const decision = await store.reserve(who, 'one-failing-id', NOW + index);
      assert.equal(decision.allowed, true);
    ...
    for (let index = 0; index < 4; index += 1) {
      const decision = await store.reserve(who, `also-rejected-${index}`, NOW + 10 + index);
      assert.equal(decision.allowed, true);
      await store.refund(who, `also-rejected-${index}`, 'unrecognized');
    }
    const blocked = await store.reserve(who, 'one-too-many', NOW + 20);
    assert.equal(blocked.allowed, false);
    assert.equal(blocked.code, 'REFUND_DAILY_LIMIT');
  });
  ...
  test(`${label}: a provider failure is refunded without counting toward the content ceiling`, async () => {
    ...
    for (let index = 0; index < 10; index += 1) {
      const decision = await store.reserve(who, `outage-${index}`, NOW + index);
      assert.equal(decision.allowed, true);
    ...
    const recovered = await store.reserve(who, 'after-recovery', NOW + 20);
    assert.equal(recovered.allowed, true);
    const usage = await store.usage(who, NOW + 20);
    assert.equal(usage.kind === 'paid' && usage.remaining24Hours, 29);
  });
  ...
  test(`${label}: a refund recorded before the cause was known counts toward nothing`, async () => {
    ...
    for (let index = 0; index < 6; index += 1) {
      await store.reserve(who, `legacy-${index}`, NOW + index);
      await store.refund(who, `legacy-${index}`, 'refunded' as never);
    }

    const allowed = await store.reserve(who, 'after-legacy', NOW + 20);
  ...
  test(`${label}: reaching the content ceiling says when it lifts`, async () => {
    ...
    for (let index = 0; index < 5; index += 1) {
      await store.reserve(who, `rejected-${index}`, NOW + index);
      await store.refund(who, `rejected-${index}`, 'unrecognized');
    }

    const blocked = await store.reserve(who, 'blocked', NOW + 10);
    assert.equal(blocked.code, 'REFUND_DAILY_LIMIT');
...
test('durable object: a reservation and its charge survive a restart of the object', async () => {
  ...
  const reserved = await runtime.store.reserve(who, 'survives-restart', NOW);
  assert.equal(reserved.duplicate, false);

  await runtime.restart();

  // The retry that follows a lost response must find its own reservation, not pay twice.
  const retried = await runtime.store.reserve(who, 'survives-restart', NOW);
  assert.equal(retried.allowed, true);
  assert.equal(retried.duplicate, true);
  const usage = await runtime.store.usage(who, NOW);
  assert.equal(usage.kind === 'paid' && usage.remaining24Hours, 29);
});
...
test('durable object: a populated database survives the migration rerun with its quota intact', async () => {
  ...
  await runtime.store.reserve(who, 'charged-before-upgrade', eventTime);
  await runtime.store.finalize(who, 'charged-before-upgrade');
  await runtime.store.reserve(who, 'rejected-before-upgrade', eventTime + 1);
  await runtime.store.refund(who, 'rejected-before-upgrade', 'unrecognized');
  await runtime.store.reserve(who, 'outage-before-upgrade', eventTime + 2);
  await runtime.store.refund(who, 'outage-before-upgrade', 'service-failure');
  const before = await runtime.store.usage(who, eventTime + 3);

  // The schema statements run again on every construction; this is that rerun.
  await runtime.restart();
  await runtime.restart();

  const after = await runtime.store.usage(who, eventTime + 3);
  ...
  const retried = await runtime.store.reserve(who, 'charged-before-upgrade', eventTime + 4);
  assert.equal(retried.duplicate, true);
});
